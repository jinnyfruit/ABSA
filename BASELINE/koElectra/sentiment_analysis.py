...
# phase info
training_phase = ["train_data",
                    "test_data",
                    "dev_data"]
data_path = [
                "/home/nlplab/Devlopment/2022_korean_AI/JiWoo/ABSA_Code/BASELINE/data/nikluge-sa-2022-train.jsonl",
                "/home/nlplab//Devlopment/2022_korean_AI/JiWoo/ABSA_Code/BASELINE/data/nikluge-sa-2022-test.jsonl",
                "/home/nlplab/Devlopment/2022_korean_AI/JiWoo/ABSA_Code/BASELINE/data/nikluge-sa-2022-dev.jsonl"
            ]
train_path = "/home/nlplab/Devlopment/2022_korean_AI/JiWoo/ABSA_Code/BASELINE/data/nikluge-sa-2022-train.jsonl"
dev_path = "/home/nlplab/Devlopment/2022_korean_AI/JiWoo/ABSA_Code/BASELINE/data/nikluge-sa-2022-dev.jsonl"
test_path = "/home/nlplab/Devlopment/2022_korean_AI/JiWoo/ABSA_Code/BASELINE/data/nikluge-sa-2022-dev.jsonl"
...
entity_property_model_path ="/home/nlplab/Devlopment/2022_korean_AI/JiWoo/ABSA_Code/BASELINE/koBERT/saved_models/category_extraction"
polarity_model_path = "/home/nlplab/Devlopment/2022_korean_AI/JiWoo/ABSA_Code/BASELINE/koBERT/saved_models/polarity_classification"
output_dir = "./output/default_path/"

test_entity_property_model_path = "/home/nlplab/Devlopment/2022_korean_AI/JiWoo/ABSA_Code/BASELINE/koElectra/saved_models/category_extractionsaved_model_epoch_10.pt"
test_polarity_model_path = "/home/nlplab/Devlopment/2022_korean_AI/JiWoo/ABSA_Code/BASELINE/koElectra/saved_models/polarity_classificationsaved_model_epoch_10.pt"


classifier_hidden_size = 256
classifier_dropout_prob =0.1 # help = dropout in classifier
...
def parse_args():
    ...
    parser.add_argument(
        "--train_data", type=str, default="/home/nlplab/Devlopment/2022_korean_AI/ajin/ABSA_ensemble_test/data/nikluge-sa-2022-train.jsonl",
        help="train file"
    )
    parser.add_argument(
        "--test_data", type=str, default="/home/nlplab/Devlopment/2022_korean_AI/ajin/ABSA_ensemble_test/data/nikluge-sa-2022-test.jsonl",
        help="test file"
    )
    parser.add_argument(
        "--dev_data", type=str, default="/home/nlplab/Devlopment/2022_korean_AI/ajin/ABSA_ensemble_test/data/nikluge-sa-2022-dev.jsonl",
        help="dev file"
    )
    parser.add_argument(
...
class SimpleClassifier(nn.Module):
    ...
    def __init__(self, num_label):  #num label이 t1의 경우 , t2의 경우 3 
        super().__init__()      #
        self.dense = nn.Linear(classifier_hidden_size, classifier_hidden_size)  # Linear가 하나의 layer라고 할 수 있음 (input차원, output차원) -> 768,768
        self.dropout = nn.Dropout(classifier_dropout_prob)      #overfitting 을 막기 위한 패털티 
        self.output = nn.Linear(classifier_hidden_size, num_label)
        #output의 shape를 맞추는 것이 중요 

    def forward(self, features):
        x = features[:, 0, :] # batch,1, hidden -> pooler output이랑 똑같음 
        x = self.dropout(x) 
        x = self.dense(x)
        x = torch.tanh(x)   #활성화함수  -> 변경가능 
        x = self.dropout(x)
        x = self.output(x)
        #원래는 여기 활성화 함수 넣어줌 -> 아래 있어서 굳이 안해도 됨 
        return x
...
class RoBertaBaseClassifier(nn.Module):
    def __init__(self, num_label, len_tokenizer):
        ...
        self.xlm_roberta.resize_token_embeddings(len_tokenizer) # 스페셜토큰 포함 (바꿔줘야 함)
    ...
    def forward(self, input_ids, attention_mask, labels=None):
        ...
        #인코딩 모델 -> 넣은 만큼 나오게 된다. 
        #last_hidden state 의 첫 토큰 pooler output -> CLS 토큰이 나오게 되는데 문장의 전체 정보 학습  [0]last h: 8 * 256 * 768  [1]pooler : 8 * 768 

        sequence_output = outputs[0]    #last Hidden
        logits = self.labels_classifier(sequence_output)

        loss = None

        if labels is not None:      #에러값 계산 
            loss_fct = nn.CrossEntropyLoss()    #여기에 시그모이드 
...
def tokenize_and_align_labels(tokenizer, form, annotations, max_len):
    ...
    for pair in entity_property_pair:
        ...
        if pd.isna(form):
            break
        tokenized_data = tokenizer(form, pair, padding='max_length', max_length=max_len, truncation=True)   # 두개 붙여서 결과값 저장 
        for annotation in annotations:
            ...
            if entity_property == pair:
                ...
                #True일 경우에만 저장 + polarity가 저장 ->false 일 경우는 알 수가 없다. 
...
def evaluation(y_true, y_pred, label_len, property):
    ...
    wandb.log({property + " accuracy: ": (sum(hit_list) / sum(count_list))})
    wandb.log({property + " macro_accuracy: ": (sum(hit_list) / 3)})
    # print(y_true)
    ...
    f1_score_value = f1_score(y_true, y_pred, average=None)
    wandb.log({property + " f1_score: ": f1_score_value})
    wandb.log({property + " f1_score_micro: ": f1_score(y_true, y_pred, average='micro')})
    wandb.log({property +" f1_score_macro: ": f1_score(y_true, y_pred, average='macro')})
    


def train_sentiment_analysis():
    if not os.path.exists(entity_property_model_path):
        os.makedirs(entity_property_model_path)
    if not os.path.exists(polarity_model_path):
        os.makedirs(polarity_model_path)

    print('train_sentiment_analysis')
    print('entity property model would be saved at ', entity_property_model_path)
    print('polarity model would be saved at ', polarity_model_path)

    print('loading train data')
    train_data = jsonlload(train_path)
    dev_data = jsonlload(dev_path)
    ...
    print('loading model')
    entity_property_model = RoBertaBaseClassifier(len(label_id_to_name), len(tokenizer))
    ...
    for _ in trange(epochs, desc="Epoch"):
        ...
        model_saved_path = entity_property_model_path + 'saved_model_epoch_' + str(epoch_step) + '.pt'
        torch.save(entity_property_model.state_dict(), model_saved_path)

        property = "eval_entity_property "
        ...
        if do_eval:
            ...
            evaluation(label_list, pred_list, len(label_id_to_name),property)
        ...
        print("Entity_polarity_Epoch: ", epoch_step)
        print("Average train loss: {}".format(avg_train_loss))
        wandb.log({"Polarity Avg train loss": avg_train_loss})

        model_saved_path = polarity_model_path + 'saved_model_epoch_' + str(epoch_step) + '.pt'
        torch.save(polarity_model.state_dict(), model_saved_path)

        property = "eval_polarity"
        ...
        if do_eval == "True":
            ...
            evaluation(label_list, pred_list, len(polarity_id_to_name),property)
...
def test_sentiment_analysis():
    ...
    model = RoBertaBaseClassifier(len(label_id_to_name), len(tokenizer))
    model.load_state_dict(torch.load(test_entity_property_model_path, map_location=device))
    model.to(device)
    model.eval()
            
    polarity_model = RoBertaBaseClassifier(len(polarity_id_to_name), len(tokenizer))
    polarity_model.load_state_dict(torch.load(test_polarity_model_path, map_location=device))
    polarity_model.to(device)
    polarity_model.eval()
...
print("training_phase start . . . \n")
train_sentiment_analysis()

#print("test_phase start . . . \n")
